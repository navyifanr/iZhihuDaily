...
//    private List<NewsModel> themeNewsList=new ArrayList<>();
    ...
    public void loadHomeData(final int themeId, int dayNum, String lastNewsId){
        if(themeId==-1){
            loadHomeNewsData(dayNum);
        }else{
           loadThemeDailyNews(themeId, lastNewsId);
        }
...
//        AppController.getInstance().addToRequestQueue(jsonObjReq, tagName);
    ...
    public void loadDrawerList(){
        RetrofitManager.builder().loadThemeList()
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Action1<ThemeList>() {
                    @Override
                    public void call(ThemeList list) {
                        List<Theme> themeList = list.getOthers();
                        if(themeList != null){
